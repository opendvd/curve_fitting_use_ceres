...
// A LossFunction is a scalar function that is used to reduce the influence of outliers
...
int main(int argc, char** argv) {
    ...
    ceres::Problem problem_with_loss;
    ...
    for (int i = 0; i < N; i++) {
        ...
        problem_with_loss.AddResidualBlock(cost_function, lossFunction, abc_with_loss);
    }
    ...
    ceres::Solver::Summary summary_with_loss;
    ceres::Solve(options, &problem, &summary);
    ceres::Solve(options, &problem_with_loss, &summary_with_loss);

    std::cout << summary.BriefReport() << std::endl;
    std::cout << summary_with_loss.BriefReport() << std::endl;


    std::clog << "===========Ceres's analytic_diff results is==========" << std::endl;
    for (auto a:abc) std::cout << a << std::endl;
    std::clog << "===========Ceres's analytic_diff with loss function results is==========" << std::endl;
    for (auto a:abc_with_loss) std::cout << a << std::endl;
